...
- [ ] 1. 统计 cue 命中的准确率。
- [ ] 2. 分离强 cue 和弱 cue。
- [ ] 3. 给 `memory_type` 增加不同 cue 权重。
- [ ] 4. 给近期聊天中的用户显式关键词更高权重。
- [ ] 5. 处理同义词和别名。
- [ ] 6. 避免过泛关键词误召回。
- [ ] 7. 给 dormant 召回设置更合理的上限和冷却。

---

## v2.3 衰减策略调参

- [ ] 1. 观察各类型记忆从 `active -> faint -> dormant -> archived` 的实际速度。
- [ ] 2. 针对 `relationship / preference / habit / constraint / fact / event / scene` 分别调半衰期。
- [ ] 3. 检查高情绪强度记忆是否保持过久。
- [ ] 4. 检查低重要度 scene 是否衰减过慢。
- [ ] 5. 检查 prompt recall boost 是否导致记忆过度复活。
- [ ] 6. 增加每次 worker run 的汇总留痕。
- [ ] 7. 后续再评估是否拆 Python worker。

---

## v2.4 记忆质量回归

- [ ] 1. 把 `summary_faint` 检查变成固定回归项。
- [ ] 2. 检查 `canonical_text / summary_short / summary_faint` 是否人称一致。
- [ ] 3. 检查重复记忆和高度相似记忆。
- [ ] 4. 检查 `semantic_key` 质量。
- [ ] 5. 检查关键词和实体词是否为空或过泛。
- [ ] 6. 检查 `emotion_profile / interaction_tendency` 是否为空壳。
- [ ] 7. 输出可人工修复的样本列表。
...
- [ ] 1. 增加按角色查看状态分布的接口。
- [ ] 2. 增加按角色查看最近 recalled / decayed / status_changed 事件的接口。
- [ ] 3. 增加单条记忆诊断接口。
- [ ] 4. 支持人工 archive / reinforce / edit 的最小安全入口。
- [ ] 5. 后续再评估是否做前端管理 UI。
...
- [ ] 1. 给 prompt 中不同层级的记忆增加更稳定的文字模板。
- [ ] 2. 让 `faint` 更像“残留印象”，但不再使用第二人称。
- [ ] 3. 让 `dormant` 明确是“被当前话题勾起的旧印象”。
- [ ] 4. 观察不同记忆数量对回复质量的影响。
- [ ] 5. 给主动消息场景和普通聊天场景设置不同记忆预算。
...
1. 先完成 v2.1 最小闭环。
2. 再用真实聊天测试 dormant cue 召回质量。
3. 根据命中样本调 cue 分数和阈值。
4. 再补 worker 运行观测的留痕深度。
5. 最后再考虑管理 UI 或 Python worker。
